...
| `GET /observations/events?runId=:id` | Flüchtiger SSE-Strom des aktiven Browserlaufs; `snapshot`, `observation` und abschließend `end` |
| `GET /observations/:id/frame` | Das neueste flüchtige JPEG des tatsächlich laufenden Playwright-Browsers; nach Laufende 404 |
...
Die Browserbeobachtung ist eine schreibgeschützte Live-Ansicht. Während eines aktiven Schritts erfasst der Runner höchstens ungefähr ein Bild pro Sekunde, hält pro Lauf nur das neueste komprimierte Bild im Speicher und schreibt diese Zwischenbilder nicht in die Datenbank. Die Metadaten nennen `capturedAt`, `sequence`, Schritt-ID, Pfad und Bezeichnung. Bei Matrixläufen hört die Oberfläche auf die ID des Elternlaufs; jedes Bild nennt zusätzlich die tatsächliche Kindlauf-ID und `rowId`. Identische aufeinanderfolgende Bilder desselben Schritts werden nicht erneut gesendet. Langsame oder getrennte SSE-Clients werden entfernt. Fehler beim Erfassen oder Übertragen eines Live-Bilds ändern weder Status noch Ergebnis des Tests.

Mit `end` verschwindet das flüchtige Bild. Danach sind ausschließlich die pro Schritt gespeicherten Screenshots, Trace und Manifest des abgeschlossenen Laufs maßgeblich. Die Live-Ansicht bietet keine Browsersteuerung und verändert Animationen oder Seiteneingaben nicht.

`workStages` hält begonnene, erledigte, übersprungene und unterbrochene Arbeitsschritte mit Zeitpunkten fest. Die lokale Prüfung des Entwurfs verwendet die Stufe `validating`. `progress` enthält während einer Erkundung die öffentliche Erläuterung, Modellrunde, Zahl der Beobachtungen und die festgelegten Grenzen. Diese Werte sind keine Restzeitschätzung. `termination` erfasst den Abbruchgrund, etwa `time_limit`, `user_cancelled`, `parent_cancelled` oder `server_restart`. Ein unspezifisches Abbruchsignal wird als `interrupted` behandelt und nicht einem Nutzer zugeschrieben. Die Felder sind optional, damit frühere Aufträge lesbar bleiben.

`progress.questions` und das Erkundungsergebnis enthalten dieselben stabilen Prüffragen. Jede Frage besitzt `id`, `text`, `requiresBrowser`, `status`, `answer`, `evidenceIds` und `knowledgeIds`. Ein späterer Modellaufruf darf Fragen nicht entfernen oder ihre Bedeutung ändern. Eine beantwortete Frage benötigt passende vorhandene Belegverweise; eine ausdrücklich verlangte Browserprüfung benötigt Browserbelege. Offene Fragen werden auch in der übergeordneten Planung geprüft und verhindern deren Fortsetzung. Die Anzeige früherer Aufträge ohne Fragenliste bleibt möglich.
