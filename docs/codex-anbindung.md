...
Provider- und Modellprofile werden lokal in einer eigenen JSON-Datei gespeichert. Standardmäßig liegt sie als `folio.settings.json` neben der mit `FOLIO_DATA_FILE` gewählten Datendatei; `FOLIO_SETTINGS_FILE` setzt einen unabhängigen Pfad. Beim ersten Lesen kopiert Folio ein vorhandenes Legacy-Profil aus der Datendatei sicher in die Einstellungsdatei und entfernt nichts aus der Quelle. So überstehen die Profile einen Reset der Szenariodaten. Für vollständig isolierte Tests sollten beide Umgebungsvariablen gesetzt werden.

Ein Auftrag hält eine Kopie der verwendeten Konfiguration als `agentConfig` fest. Auch seine Korrekturversuche und zugehörigen Agentenschritte verwenden diese Kopie. Ein späterer Wechsel der Einstellungen verändert laufende Aufträge nicht. Alte Aufträge ohne diesen Zusatz bleiben lesbar.
...
Die Annahme prüft Ausgangsrevision und Fachfingerprint erneut und speichert den neuen Ablauf samt neuen Definitionen und Wissensverweisen atomar. Veraltete Vorschläge können den aktuellen Stand nicht überschreiben. Der geänderte Ablauf benötigt eine neue Fachfreigabe. Diese startet danach automatisch die technische Runde. Frühere Laufnachweise behalten ihre ursprüngliche Momentaufnahme.

## Titel und kurze Beschreibung

Ein neuer Auftrag wird sofort als „Neuer Testfall“ gespeichert. Vor der Erkundung startet ein eigener Teilauftrag zur Benennung. Er erhält die Anforderung und gegebenenfalls den bisherigen Titel, erzeugt einen kurzen deutschen Titel, eine Zusammenfassung des Testziels und Themenbegriffe. Diese Texte beschreiben das Ziel, nicht einen bereits erfolgreichen Test.

Gibt die Anforderung ausdrücklich einen Titel vor, soll der Benennungsagent ihn wortgetreu übernehmen. Die Antwort muss dafür die entsprechende Textstelle zitieren; die Anwendung prüft, dass sie tatsächlich in der Anforderung steht. Bereits brauchbar benannte Entwürfe werden bei einer unveränderten Wiederholung nicht erneut umbenannt. Eine inzwischen bearbeitete Fassung darf auch durch ein später eintreffendes Benennungsergebnis nicht überschrieben werden.

Die Benennung verwendet dieselbe für den Auftrag ausgewählte Modellkonfiguration wie Wissensprüfung und fachliche Planung. Provider, Programm, Modell-Slug, Zusatzargumente und Einstellungsrevision stammen aus demselben unveränderlichen Konfigurationssnapshot. Schlägt die Benennung fehl, bleibt der Entwurf mit seinem bisherigen Titel bearbeitbar und die fachliche Arbeit kann fortgesetzt werden.
...
Ein HTTP-Status ist nur belegt, wenn `capture` direkt am ausführenden UI-Klick die Antwort erfasst und den erwarteten Status prüft. Ein gesperrter Button sendet keine Anfrage und kann deshalb keinen `403`-Status über die Buttonbedienung nachweisen. Ein Sichtbarkeitscheck, auch mit einer Bedingung wie `expectedHttpStatus`, ist dafür kein HTTP-Nachweis. Bleibt eine technische Fähigkeit damit nicht ausführbar, zeigt Schritt 4 „Technische Fragen klären“ den Abschnitt „Mit KI überarbeiten“. Dort wählt der Mensch das „Lokale Modell für die fachliche Überarbeitung“, startet mit „Mit KI überarbeiten“ einen Vorschlag und entscheidet danach über die Übernahme. „Manuell im Ablauf bearbeiten“ bleibt verfügbar. Erst „Freigeben und technisch prüfen“ startet die technische Runde für den geänderten Stand.
...
Ein Dublettenfund führt zur sichtbaren Entscheidung. Ein gleichwertiger vorhandener Block kann übernommen werden; daraus entsteht eine neue fachliche Revision zur erneuten Prüfung. Eine notwendige Erweiterung bleibt als konkrete Fachaufgabe sichtbar. Meldet die technische Planung eine nicht unterstützte Fähigkeit, zeigt Schritt 4 „Technische Fragen klären“ den kontrollierten fachlichen Überarbeitungsweg. Der Mensch prüft und übernimmt den Vorschlag ausdrücklich und gibt den neuen Stand mit „Freigeben und technisch prüfen“ erneut frei. Manuelle Änderungen im Scratch-Editor bleiben möglich. Die Anwendung ersetzt dabei keine Fachsemantik still im Hintergrund.
