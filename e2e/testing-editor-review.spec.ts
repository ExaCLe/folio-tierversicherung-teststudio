...
test('Menschen ergänzen eine fehlende Fähigkeit und erweitern ihr typisiertes Schema als neue Version', async ({ page, request }, testInfo) => {
  ...
  const catalog = await (await request.get('/api/testing/catalog')).json() as TestingCatalog;
  await openEditor(page, current);
  await page.getByRole('button', { name: 'Fehlenden Block definieren', exact: true }).click();
  ...
  await page.getByLabel('Blockbibliothek durchsuchen', { exact: true }).fill(name);
  await page.locator('.t-definition-card').filter({ has: page.getByRole('heading', { name, exact: true }) }).click();
  await page.getByRole('button', { name: 'In Testfall verwenden', exact: true }).click();
