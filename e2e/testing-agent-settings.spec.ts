...
test('Ein manuell ergänzter Block erklärt eine falsche Ergebnisquelle und springt auch zum fünften verschachtelten Fehlerfeld', async ({ page, request }, testInfo) => {
  ...
  await page.getByRole('dialog').getByRole('option').filter({ hasText: 'Ergebnisquelle prüfen' }).click();
  await page.getByLabel('Einfügestelle', { exact: true }).selectOption('within');
  await page.getByRole('dialog').getByRole('button', {name:'Am Ende anhängen',exact:true}).click();
  await page.getByRole('button', { name: 'Speichern', exact: true }).click();
  await page.getByRole('tab', { name: 'Technik', exact: true }).click();
  await page.locator('.t-validation-counters button.error').click();
  const issues = page.getByRole('dialog', {name:'Hinweise zum Testfall'}).locator('.t-review-issue').filter({ hasText: 'Ergebnisquelle prüfen' }).filter({ hasText: 'Kunde' });
