...
export function validateTestingBinding(input: unknown, catalog: TestingCatalog): TestingTechnicalBinding {
  ...
  for (const item of binding.recipe ?? []) {
    ...
    if (item.proof && !['expectText', 'expectVisible', 'expectEnabled', 'expectDisabled'].includes(item.op)) throw new Error('Prüfergebnisse dürfen nur nach einer echten UI-Assertion erfasst werden.');
  }
