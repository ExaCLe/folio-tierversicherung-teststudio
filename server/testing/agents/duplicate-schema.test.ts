...
import { businessPrompt, duplicatePrompt, reusePrompt, technicalPrompt } from './prompts';
import { explorationPromptContext } from './exploration';
...
test('Fachplanung trennt Anforderungen von beobachtbaren Belegen und übernimmt ambienten Kontext',()=>{
  const prompt=businessPrompt('Als Prüfer darf ich nicht freigeben.');
  assert.match(prompt,/fachliche Anforderung vom konkreten beobachtbaren erwarteten Beleg/);
  assert.match(prompt,/HTTP-Status ist nur zulässig.*capture/s);
  assert.match(prompt,/Kontextblöcke etablieren einen ambienten Ausführungskontext/);
  assert.match(prompt,/ausdrücklicher Kontextwechsel.*zulässig/s);
  assert.match(prompt,/Als Prüfer darf ich nicht freigeben/);
});

test('Erkundung veröffentlicht nur tatsächlich verfügbare Belegarten',()=>{
  const context=explorationPromptContext('Der Zugriff soll gesperrt sein.',catalog,0,[],[]);
  assert.deepEqual(context.runtimeCapabilities.actions,['goto','click','fill','select','check','readSnapshot']);
  assert(context.runtimeCapabilities.unavailableEvidence.some(item=>item.includes('HTTP-Status')));
  assert.match(context.clarificationPolicy.evidenceBoundary,/Unsichtbarkeit|unsichtbar/);
});

test('Technische und Wiederverwendungsprüfung erkennen nur unnötige Kontextduplikate',()=>{
  for(const prompt of [technicalPrompt(),reusePrompt()]){
    assert.match(prompt,/unnötig identische Angaben.*ambient aktiven Kontext/s);
    assert.match(prompt,/wiederverwendbare Definitionen dürfen notwendige explizite Kontextparameter behalten/s);
    assert.match(prompt,/Kontextwechsel.*Kontextassertions.*zulässig/s);
  }
});
