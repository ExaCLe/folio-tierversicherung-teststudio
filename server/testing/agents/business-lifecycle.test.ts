...
else if(schema.properties.knowledgeIds){const docs=JSON.parse(readFileSync('wissen.json','utf8'));const unanswered=process.env.FOLIO_BUSINESS_MODE==='question';response={decision:'finish',explanation:'Synthetische Wissensprüfung, keine KI-Erkundung.',knowledgeIds:[docs[0].id],gaps:unanswered?['Welche zusätzliche fachliche Regel gilt?']:[],questions:[{id:'anforderung-1',text:'Welche fachlichen Regeln gelten für die Anforderung?',kind:'research',why:'',requestQuote:'',requiresBrowser:false,status:'answered',answer:'Die synthetische Wissensprüfung verwendet die bereitgestellte Regel.',knowledgeIds:[docs[0].id],evidenceIds:[]},...(unanswered?[{id:'zusatzregel',text:'Welche zusätzliche fachliche Regel gilt?',kind:'clarification',why:'Ohne diese Regel fehlt der fachliche Sollwert für den Ablauf.',requestQuote:'',requiresBrowser:false,status:'open',answer:'',knowledgeIds:[],evidenceIds:[]}]:[])],action:null,findings:[]};}
