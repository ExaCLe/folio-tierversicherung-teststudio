...
export const EXPLORATION_SCHEMA = object({ decision: { type: 'string', enum: ['explore', 'act', 'finish'] }, explanation: text,
  ...
  questions: { type: 'array', items: object({ id: text, text, requiresBrowser: { type: 'boolean' }, status: { type: 'string', enum: ['open', 'answered'] }, answer: text, evidenceIds: texts, knowledgeIds: texts }) },
...
export function validateExplorationQuestions(questions: TestingExplorationQuestion[], previous: TestingExplorationQuestion[],
  ...
  for (const question of questions) {
    ...
    ids.add(question.id);
    if (question.knowledgeIds.some(id => !knowledge.some(item => item.id === id))) issues.push(`${question.id}: unbekannter Wissensbeleg.`);
