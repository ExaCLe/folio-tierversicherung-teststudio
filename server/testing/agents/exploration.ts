...
/** Repair provider-authored audit metadata without changing the requirement,
 * its asserted answer, or any evidence references. Quotes are optional because
 * the complete request is already retained as the authoritative source. */
export function normalizeExplorationQuestions<T extends TestingExplorationQuestion>(questions: T[], request: string): T[] {
  return questions.map(question => {
    if (question.kind !== 'requirement') return question;
    const requestQuote = question.requestQuote?.trim() ?? '';
    return { ...question, why: '', requestQuote: requestQuote && request.includes(requestQuote) ? requestQuote : '' } as T;
  });
}
...
export function validateExplorationQuestions(questions: TestingExplorationQuestion[], previous: TestingExplorationQuestion[],
  ...
  for (const question of questions) {
    ...
    if (question.kind === 'requirement') {
      ...
      if (question.requestQuote?.trim() && !request.includes(question.requestQuote)) issues.push(`${question.id}: requestQuote darf ausschließlich einen wörtlichen Auszug aus der Nutzeranforderung enthalten.`);
    ...
    for (round = 0; round <= LIMIT_ACTIONS; round++) {
      ...
      ({ parsed: reply } = await reviewWithCodex({ id: `${input.id}-observation-${round}`, model: input.model, signal: controller.signal, onEvent: input.onEvent,
        ...
        validate: raw => {
          ...
          const value = { ...parsed.data, questions: normalizeExplorationQuestions(parsed.data.questions, input.request) }, issues: string[] = [];
