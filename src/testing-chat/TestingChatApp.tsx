...
function TaskMetrics({ task }: { task: PublicTask }) {
  ...
  return <span className="tc-task-metrics">{visibleElapsed != null && <span>{formatDuration(visibleElapsed)}</span>}{metrics.apiRequestCount != null ? <span>{metrics.apiRequestCount} API-Anfragen</span> : <span title="Der Anbieter hat für diesen Auftrag keine Anzahl einzelner API-Anfragen gemeldet.">API-Anfragen unbekannt</span>}{tokenMetric('Eingabe', inputTokens)}{tokenMetric('Cache', metrics.cachedInputTokens)}{tokenMetric('Ausgabe', metrics.outputTokens)}{tokenMetric('Gesamt', metrics.totalTokens)}</span>;
}
