...
export function WorkspaceNavigation({ view, open, onToggle, onNavigate, onResume, scenarioId }: { view: WorkspaceView; open: boolean; onToggle: () => void; onNavigate: (view: WorkspaceView) => void; onResume: () => void; scenarioId?: string }) {
  ...
  const link = (id: WorkspaceView, label: string, Icon: typeof Plus) => <a key={id} href={workspacePath(id)} aria-current={view === id ? 'page' : undefined} onClick={event => { event.preventDefault(); onNavigate(id); }}><Icon size={16}/>{label}</a>;
  return <header className="t-topbar t-workspace-topbar">
    <a className="t-brand" href="/testing/new" onClick={event => { event.preventDefault(); onNavigate('start'); }}><span className="t-brand-mark"><i/><i/><i/></span><strong>folio</strong><span>TESTSTUDIO</span></a>
    <nav aria-label="Teststudio-Navigation">
      <a href={scenarioId ? `/testing/editor/${encodeURIComponent(scenarioId)}` : '/testing/new'} aria-current={view === 'start' || view === 'editor' ? 'page' : undefined} onClick={event => { event.preventDefault(); scenarioId ? onResume() : onNavigate('start'); }}><Plus size={16}/>Testfall erstellen</a>
      {link('scenarios', 'Alle Testfälle', Layers3)}
      <details className="t-resource-menu"><summary aria-current={secondary ? 'page' : undefined}><BookOpen size={16}/>Bibliothek & Wissen<ChevronDown size={13}/></summary><div>{link('library', 'Blockbibliothek', Layers3)}{link('knowledge', 'Wissensbasis', BookOpen)}{link('graph', 'Abhängigkeiten', Layers3)}{link('runs', 'Alle Ausführungen', Layers3)}{link('method', 'So funktioniert es', BookOpen)}</div></details>
