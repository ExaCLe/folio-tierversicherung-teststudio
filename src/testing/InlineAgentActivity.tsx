...
import { StageDialogContent } from './StageDialogContent';
...
export function InlineAgentActivity({ job, related = [], onCancel, children, knowledge = [], catalog }: { job: TestingAgentJob; related?: TestingAgentJob[]; onCancel?: () => void; children?: ReactNode; knowledge?: TestingKnowledgeDocument[]; catalog?: TestingCatalog }) {
  ...
  return <section className={`t-inline-activity ${model.running ? 'running' : failed ? 'failed' : ''}`} aria-label="Aktueller Arbeitsstand">
    ...
    <ol className={`t-agent-stages ${job.phase === 'technical' ? 'parallel' : 'sequential'}`} aria-label="Arbeitsschritte der KI">{model.stages.map(stage => {
      const events = stage.job ? branchEvents(stage.job, related, stage.id, stage.workStage) : [];
      const details = stage.job ? resultLines(stage.id, stage.job, catalog) : [];
      const summary = meaningfulExplanation(stage.summary);
      const questions = stage.id === 'exploring' ? (stage.job?.result as { questions?: unknown[] } | undefined)?.questions ?? stage.job?.progress?.questions : undefined;
      const canOpen = !!stage.job && (!!summary || events.length > 0 || details.length > 0 || !!questions?.length);
      ...
      return <li key={stage.key} data-stage={stage.id} data-state={stage.state}>{canOpen ? <button type="button" className="t-stage-card" aria-label={`${stage.label}: Ergebnis und Erläuterungen ansehen`} onClick={() => setOpenStage(stage.key)}>{content}</button> : <div className="t-stage-card">{content}</div>}{openStage === stage.key && <Modal title={stage.label} subtitle="Ergebnis und Erläuterungen dieses Arbeitsschritts" onClose={() => setOpenStage(undefined)} wide={stage.id === 'exploring'}><StageDialogContent stageId={stage.id} job={stage.job!} summary={summary} details={details} events={events} knowledge={knowledge}/></Modal>}</li>;
    })}</ol>
    {model.running && <div className="t-activity-current" aria-live="polite">{model.current.length ? model.current.map(stage => <div key={stage.key}>{model.current.length > 1 && <strong>{stage.label}</strong>}<p>{(stage.job?.progress?.stage === stage.id && stage.job.progress.status !== 'finished' ? meaningfulExplanation(stage.job.progress.summary) : undefined) ?? meaningfulExplanation(stage.summary) ?? branchEvents(stage.job ?? job, related, stage.id, stage.workStage).at(-1)?.message ?? 'Der nächste Arbeitsschritt wird vorbereitet.'}</p></div>) : <p>{model.waitingAgents ? 'Die Bearbeitung wartet auf einen freien lokalen KI-Platz.' : 'Der Auftrag wartet auf den nächsten Arbeitsschritt.'}</p>}<small>{job.phase === 'technical' ? 'Technische Vorbereitung und Bausteinvergleich können gleichzeitig arbeiten. Danach folgt der Probelauf.' : 'Die Schritte werden nacheinander bearbeitet. Ein Zwischenergebnis beendet den Auftrag noch nicht.'}</small></div>}
    ...
    <div className="t-activity-details"><DetailsButton label="Erläuterungen und Verlauf ansehen"><p className="t-caption">{toolLabel}</p><p className="t-caption">Öffentliche Statusmeldungen und Ergebnisse. Die Auftragskoordination zählt nicht als zusätzlicher KI-Agent.</p>{!model.events.length && <p>Es liegt noch keine öffentliche fachliche Erläuterung vor.</p>}<ol className="t-public-events">{model.events.map(event => <li key={event.id}><time>{new Date(event.at).toLocaleTimeString('de-DE')}</time><div><strong>{event.source}</strong><p>{event.message}</p></div></li>)}</ol><div className="t-activity-diagnostics"><span>{jobModelLabel(job)}</span><span>{toolLabel} im verfügbaren Verlauf</span><span>Gespeicherter Fachstand: Revision {job.scenarioRevision ?? '–'}</span></div></DetailsButton></div>
  </section>;
}
