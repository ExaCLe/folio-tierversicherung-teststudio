...
export function DefinitionDialog({ definition, catalog, onSave, onClose, saveLabel, insertionHint }: { saveLabel?: string; insertionHint?: string; definition?: TestingBlockDefinition; catalog: TestingCatalog; onSave: (definition: TestingBlockDefinition, newKnowledge?: TestingKnowledgeDocument[]) => Promise<void>; onClose: () => void }) {
  ...
  return <Modal title={definition ? 'Gemeinsame Blockdefinition aktualisieren' : 'Einen fachlichen Block definieren'} subtitle={definition ? 'Die neue aktuelle Definition gilt für alle bearbeitbaren Testfälle. Historische Ausführungen behalten ihre gespeicherten Nachweise.' : 'Beschreibe den Block und seine Eingaben. Interne Zuordnungen entstehen automatisch.'} onClose={() => { if (!busy) onClose(); }} wide>
    <div className="t-definition-form t-definition-simple">
      ...
      <footer className="t-dialog-actions"><button className="t-button" onClick={onClose} disabled={busy}>Abbrechen</button><button className="t-button primary" onClick={save} disabled={busy || !!schemaIssue || knowledgeIssue || !draft.name.trim() || !draft.kind || incompleteInputs(draft.inputs)}>{busy ? <Spinner label="Wird gespeichert"/> : saveLabel ?? (definition ? 'Definition aktualisieren' : 'Definition speichern')}</button></footer>
    </div>
  </Modal>;
}
