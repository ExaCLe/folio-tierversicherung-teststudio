...
export function TestingApp() {
  ...
  async function approve() { const saved = await saveDraft(); if (!saved) return; await action('approve', async () => { const checked = await testingApi<TestingCompiledScenario>(`/scenarios/${encodeURIComponent(saved.id)}/compile`); setCompiled(checked); if (checked.issues.some(issue => issue.severity === 'error')) { setValidationOpen(true); return; } await testingPost(`/scenarios/${encodeURIComponent(saved.id)}/approve`, { revision: saved.revision }); await refresh(); const result = await testingApi<TestingCompiledScenario>(`/scenarios/${encodeURIComponent(saved.id)}/compile`); if (draftRef.current?.id === saved.id && draftRef.current.revision === saved.revision && JSON.stringify(draftRef.current) === savedRef.current) setCompiled(result); followProgress(); setNotice('Der fachliche Ablauf ist freigegeben. Jetzt kann die technische Umsetzung beginnen.'); }); }
  ...
  function useDefinitionVersion(definition: TestingBlockDefinition) {
    ...
    try { editSelectedScope(migrateDefinition(selectedBlocks, selected, catalog, definition, parkedIndex >= 0 ? {} : current.parameters)); setNotice(`Dieser Schritt verwendet jetzt Version ${definition.version}. Prüfe seine Eingaben und speichere den Testfall.`); }
  ...
  async function saveDefinition(definition: TestingBlockDefinition, newKnowledge: TestingKnowledgeDocument[] = []) {
    ...
    if (target) {
      const current = draftRef.current;
      const targetParked = layout?.parkedStacks?.findIndex(stack => flattenBlocks(stack, next.catalog).some(item => item.path === target.path)) ?? -1;
      const targetBlocks = targetParked >= 0 ? layout!.parkedStacks![targetParked] : current?.blocks ?? [];
      const usage = current?.id === target.scenarioId ? flattenBlocks(targetBlocks, next.catalog).find(item => item.path === target.path) : undefined;
      ...
      if (targetParked >= 0) { const changed = migrateDefinition(targetBlocks, target.path, next.catalog, definition, {}); setLayout(old => { if (!old?.parkedStacks) return old; const stacks = old.parkedStacks.map((stack, index) => index === targetParked ? changed : stack); return { ...old, parkedStacks: stacks, parkedBlocks: stacks.flat() }; }); return; }
  ...
  return <AgentSettingsContext.Provider value={settings}><div className={`testing-app ${mobileNav ? 'nav-open' : ''}`}><a className="t-skip" href="#testing-main">Zum Inhalt</a><WorkspaceNavigation view={view} open={mobileNav} onToggle={() => setMobileNav(!mobileNav)} onNavigate={navigate} scenarioId={resumeScenario?.id} onResume={() => resumeScenario && openScenario(resumeScenario.id)} /><main id="testing-main">{error && <div className="t-global-notice"><Notice tone="error" onClose={() => setError('')}>{error}</Notice></div>}{notice && <div className="t-global-notice"><Notice tone="success" onClose={() => setNotice('')}>{notice}</Notice></div>}{!data ? <div className="t-loading-page">{error ? <button className="t-button" onClick={() => { setError(''); refresh().catch(cause => setError(messageOf(cause))); }}>Erneut verbinden</button> : <Spinner label="Teststudio wird geöffnet" />}</div> : <>
    ...
    {view === 'editor' && (!draft ? <Empty title="Testfall wird geöffnet"/> : <div className="t-editor-page t-linear-workspace">
      ...
      <WorkspaceFrame phase={workspacePhase} available={availablePhases} onSelect={selectPhase} title={workspaceTitle} description={workspaceDescription} action={workspaceAction}>
        ...
        {displayedJob && (workspacePhase === 1 || workspacePhase === 3) && <section className="t-workspace-job"><InlineAgentActivity knowledge={data.catalog.knowledge} job={displayedJob} related={displayedRelatedJobs} onCancel={jobRunning && displayedJob.id === workspaceJob?.id ? () => void action('cancel', async () => { const next = await testingPost<TestingAgentJob>(`/jobs/${encodeURIComponent(displayedJob.id)}/cancel`, {}); setActiveJob(next); await refresh(); }) : undefined}>{!jobRunning && displayedJob.result !== undefined && <><AgentOutcome result={displayedJob.result}/><DuplicateReview job={displayedJob} catalog={data.catalog} onResolved={async scenario => { const next = await refresh(); loadScenario(scenario, next); navigate('editor', scenario.id); }}/></>}</InlineAgentActivity></section>}
