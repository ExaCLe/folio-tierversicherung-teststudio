...
export function TestingApp() {
  ...
  function navigate(next: View, id?: string) {
    ...
    const path = next === 'start' ? '/testing' : `/testing/${next}${targetId ? `/${encodeURIComponent(targetId)}` : ''}`;
  ...
  async function run() { if (!draft || dirty) return; await action('run', async () => { const next = await testingPost<TestingRun>(`/scenarios/${encodeURIComponent(draft.id)}/run`, { revision: draft.revision }); if (draftRef.current?.id === next.scenarioId) setActiveRun(next); }); }
  ...
  function addDefinition(definition: TestingBlockDefinition, target?: InsertionTarget, catalog = dataRef.current?.catalog) {
    const current = draftRef.current;
    if (!current || !catalog) { setNotice('Öffne zuerst einen Testfall oder lasse einen fachlichen Entwurf erstellen.'); navigate('start'); return; }
  ...
  async function saveDefinition(definition: TestingBlockDefinition) {
    ...
    const editedPath = selected;
    const editedScenarioId = draftRef.current?.id;
    ...
    if (draftRef.current?.id === editedScenarioId && editedPath && definition.supersedes) {
      setDraft(current => current ? { ...current, blocks: updateBlockAtPath(current.blocks, editedPath, next.catalog, block => testingVersionKey(block.definition) === testingVersionKey(definition.supersedes!) ? { ...block, definition: { id: definition.id, version: definition.version } } : block) } : current);
    }
    setNotice(`„${definition.name}“ wurde als Version ${definition.version} gespeichert.`);
  }
